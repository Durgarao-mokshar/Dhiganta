				...
				<div className="grid grid-cols-1 lg:grid-cols-2 max-w-7xl w-full mx-auto">
					...
					{success ? (
						<div className="w-full h-full flex flex-col justify-center items-center text-center py-32">
							...
							<p className="text-3xl text-gray-900">your form has been submitted!</p>
						</div>
					) : (
						<form onSubmit={submitForm} autoComplete="off" className="w-full h-full flex flex-col">
							<div className="w-full h-full grid grid-cols-1 lg:grid-cols-2 gap-1 p-4">
								<div className="relative z-0 w-full m-1">
									<input
										...
										className="block py-3 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b border-white appearance-none dark:text-gray-900 dark:border-white dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer"
										placeholder=" "
										value={name}
									...
									<label
										...
										className="absolute text-sm text-gray-500 dark:text-gray-900 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-orange-600 peer-focus:dark:text-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
									>
										Your Name
									</label>
								</div>
										...
										<div
											onClick={() => {
												if (service.find((value) => value._id === ele._id)) {
													setService(service.filter((value) => value._id !== ele._id))
												} else {
											...
											className={`px-6 py-3 cursor-pointer border-2 rounded-full flex justify-center items-center text-blue-900 font-semibold my-2 mr-4 hover:scale-110 hover:border-[#fde047] hover:bg-orange-400 ${
												service.find((service) => service._id === ele._id) ? 'bg-[#F2751A] border-[#F2751A]' : 'border-[#FDE047]'
											}`}
										>
							...
							<div className="flex items-center justify-start p-4">
								<input required type="checkbox" className="accent-[#F2751A] border-2 border-gray-200" />
								<p className="text-gray-900 pl-4">
									I agree with your <span className="text-[#F2751A]">privacy policy</span>
								</p>
							</div>
							<div className="p-4">
								<div>
									...
									>
										...
										<span className={`${loading ? '' : 'ml-2'}`}>{loading ? 'loading...' : 'submit'}</span>
									</button>
								</div>
							</div>
						</form>
